  ...
  try {
    ...
    assert.ok(
      templates.templates.some(
        (template) => template.key === "simplex" && template.type === "messaging"
      )
    );
    assert.ok(
      templates.templates.some(
        (template) => template.key === "protonmail" && template.type === "mail"
      )
    );
    ...
    assert.ok(
      created.provisioningDraft.workloads.every(
        (workload) => workload.isolation === "firecracker_microvm"
      )
    );
    assert.ok(
      created.provisioningDraft.workloads.every(
        (workload) => workload.secretsReleaseAllowed === false
      )
    );
    ...
    assert.deepEqual(
      created.baselineProvisioning.vps.map((vps) => vps.role),
      ["G1", "G2", "WORKLOAD"]
    );
    assert.equal(created.baselineProvisioning.firecrackerPlan.workloads.length, 3);
    assert.ok(
      created.baselineProvisioning.firecrackerPlan.workloads.every(
        (workload) => workload.status === "planned"
      )
    );
    ...
    assert.ok(
      app.services.audit
        .list()
        .some((event) => event.action === "operator_provisioning.local_lab_vps_created")
    );
  } finally {
    await close();
  }
