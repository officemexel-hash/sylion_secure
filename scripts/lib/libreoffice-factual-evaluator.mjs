...
function visualMarkerAccepted(marker) {
  return ["libreoffice", "libreoffice_writer", "libreoffice_calc", "libreoffice_start_center"].includes(value(marker).toLowerCase());
}

function visualMarkerRejected(marker) {
  return ["generic_browser", "download_page", "new_tab", "about_blank", "only_process"].includes(value(marker).toLowerCase());
}
...
export function evaluateLibreOfficeFactualState({
  ...
  if (streamSession?.security?.fileIngressEgress && !String(streamSession.security.fileIngressEgress).includes("cdr")) {
  ...
  const documentWorkflowPassed = documentProbe?.status === "passed"
  ...
  const cdrPassed = cdrProbe?.cdrRequired === true
    && cdrProbe?.ingressEgressBlockedWithoutDecision === true;
  ...
  const failed = blockers.some((blocker) => blocker.startsWith("wrong_office_marker:"))
  ...
  const checks = {
    uiVisible: visualPassed
      ? check("passed", "LibreOffice UI marker observed through workload stream", visualProbe?.note || null, visualProbe?.mode || null)
    ...
    documentWorkflow: documentWorkflowPassed && cdrPassed
      ? check("passed", "Non-sensitive document workflow metadata passed with CDR boundary present", documentProbe?.note || null, documentProbe?.mode || null)
      : check(failed ? "failed" : "blocked", null, "LibreOffice document workflow or CDR boundary is not proven")
  };
  return {
