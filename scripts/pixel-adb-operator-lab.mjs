...
const adbPath =
  process.env.SYLION_ADB_PATH || join(process.cwd(), ".deploy", "platform-tools", "adb.exe");
const outputDir = join(
  process.cwd(),
  "docs",
  "admin-panel-v2",
  "test-artifacts",
  "step3-24-pixel-adb-operator-lab"
);
...
async function ensureAdb() {
  try {
    ...
    throw new Error(
      `ADB not found at ${adbPath}. Install Android platform-tools or set SYLION_ADB_PATH.`
    );
  }
}
...
async function requireAuthorizedPixel() {
  ...
  if (unauthorized) {
    throw new Error(
      `Pixel ${unauthorized.serial} is visible but unauthorized. Confirm the USB debugging prompt on the phone and rerun.`
    );
  }
  const device = devices.find((candidate) => candidate.state === "device");
  if (!device) {
    throw new Error(
      "No authorized ADB device found. Connect the Pixel, unlock it, and authorize USB debugging."
    );
  }
  return device;
}
...
async function run() {
  ...
  const tenant = await client.createTenant({
    name: `Step 3.24 Pixel ADB Tenant ${stamp}`,
    tier: "PRO"
  });
  ...
  const posture = {
    ...
    sdk,
    securityPatch,
    fingerprintHash: await crypto.subtle
  ...
  const [me, vpn, connectionPath, signalExecution, vpnInstall, stream, devices, profiles] =
    await Promise.all([
  ...
  const summary = {
    ...
    localLabId:
      operator.baselineProvisioning.vps?.length === 3 ? pipeline.localLab?.id || null : null,
    ...
    pixelProfileAvailable: profiles.profiles.some(
  ...
  const labHealthy =
    summary.openedOperatorPortalOnPixel &&
  ...
  const humanEvidence = await writeHumanEvidenceSummary(
    outputDir,
    {
      ...
      pathTested:
        "Pixel ADB -> local reverse tunnel -> operator panel local lab -> simulated G1/G2/WORKLOAD metadata",
      expectedBehavior:
        "A connected Pixel opens the operator panel in lab mode, receives only thin-client metadata, and does not store operational data on the terminal.",
      preconditions: [
      ...
      nextRequiredAction:
        "Run step3-40 live Pixel human regression and compare with live path evidence."
    },
