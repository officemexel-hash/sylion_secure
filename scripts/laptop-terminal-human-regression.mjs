...
const outputDir = join(process.cwd(), "docs", "admin-panel-v2", "test-artifacts", "step3-86-laptop-terminal-human-regression");
...
async function seedLaptopOperator(client) {
  ...
  const tenant = await client.createTenant({ name: `Step 3.86 Laptop Tenant ${stamp}`, tier: "PRO" });
  ...
  return { tenant, operator: operator.operator, pipeline, environment: environment.environment, laptop: laptop.device, session: sessionPayload.session };
}
...
async function run() {
  ...
  const [me, connectionPath, signalExecution, stream, terminalProfiles] = await Promise.all([
    ...
    operatorRequest(seeded.session.token, `/operator-api/streaming-profile?width=${viewport.width}&height=${viewport.height}&dpr=${viewport.dpr}`),
  ...
  try {
    ...
    const adminText = visible(await page.locator("body").innerText().catch(() => ""));
    ...
    const operatorText = visible(await page.locator("body").innerText().catch(() => ""));
  ...
  if (connectionPath.path.state !== "ready") {
    issues.push(`Connection path state is ${connectionPath.path.state}, expected ready for lab parity.`);
  }
  ...
  const summary = {
    ...
    screenshots: Object.fromEntries(Object.entries(screenshots).map(([key, path]) => [key, repoRelativePath(path)])),
    issues,
  ...
  const humanEvidence = await writeHumanEvidenceSummary(outputDir, {
    ...
    pathTested: "Laptop browser -> operator panel -> local lab G1/G2/WORKLOAD metadata -> workload stream selector",
    expectedBehavior: "A laptop terminal can open the admin panel and operator workload controls as a thin client without storing operational data locally.",
    preconditions: [
    ...
    nextRequiredAction: "Run laptop live path test after G1/G2/workload route and stream gateway are fully evidenced."
