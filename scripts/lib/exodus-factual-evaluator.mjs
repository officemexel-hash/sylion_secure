...
function forbiddenProbeFields(input, path = []) {
  ...
  return Object.entries(input).flatMap(([key, nested]) => {
    ...
    const normalized = String(key)
      ...
      .toLowerCase();
    const own =
      /(password|secret|token|api_key|otp|sms|seed|mnemonic|private_key|wallet_seed|wallet_secret|wallet_file|recovery_phrase)/.test(
        normalized
      )
        ? [current.join(".")]
        : [];
...
export function evaluateExodusFactualState({
  ...
  const walletWorkflowPassed =
    walletProbe?.status === "passed" &&
  ...
  const failed =
    blockers.some((blocker) => blocker.startsWith("wrong_exodus_marker:")) ||
  ...
  const checks = {
    uiVisible: visualPassed
      ? check(
          "passed",
          "Exodus UI marker observed through workload stream",
    ...
    walletWorkflow: walletWorkflowPassed
      ? check(
          "passed",
          "Test-only Exodus wallet workflow metadata passed without wallet data",
          walletProbe?.note || null,
          walletProbe?.mode || null
        )
      : check(
          failed ? "failed" : "blocked",
          null,
          "Exodus wallet workflow is not proven without wallet data"
        ),
    riskAcceptance: riskAccepted
      ? check(
          "passed",
          "Operator risk acceptance metadata is recorded",
          riskProbe?.note || null,
          riskProbe?.mode || null
        )
