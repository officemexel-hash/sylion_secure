...
test("Step 3.72 operator account bootstrap records pass/fail evidence without secrets or production claim", async () => {
  ...
  try {
    ...
    const overview = await operatorRequest(
      ...
      "/operator-api/account-bootstrap"
    );
    ...
    assert.equal(
      overview.payload.bootstrap.guardrails.adminQaReviewRequiredForProductionReadiness,
      true
    );

    const created = await operatorRequest(
      baseUrl,
      seeded.session.token,
      "/operator-api/account-bootstrap/sessions",
      {
        method: "POST",
        body: {
          appKey: "signal",
    ...
    assert.equal(
      created.payload.session.humanHandoff.privateInputEntry,
      "directly_inside_workload_ui_only"
    );
    assert.ok(created.payload.session.humanHandoff.neverCollect.includes("otp_or_sms_code"));
    assert.ok(
      created.payload.session.humanHandoff.orderedSteps.some((step) =>
        /directly in the workload UI/i.test(step)
      )
    );
    assert.equal(created.payload.session.productionExecutionAllowed, false);

    const incomplete = await operatorRequest(
      baseUrl,
      seeded.session.token,
      `/operator-api/account-bootstrap/sessions/${created.payload.session.id}/evidence`,
      {
        method: "POST",
        body: {
          result: "blocked",
    ...
    const passed = await operatorRequest(
      baseUrl,
      seeded.session.token,
      `/operator-api/account-bootstrap/sessions/${created.payload.session.id}/evidence`,
      {
        ...
        body: {
          ...
          checks: {
            ...
            accountBootstrap: {
              status: "passed",
              evidence: "Disposable account linked by operator"
            },
    ...
    const promoted = await client.request(
      `/release/account-bootstrap-evidence/${passed.payload.session.id}/promote`,
      {
        method: "POST",
        body: {
    ...
    assert.ok(
      audit.some((event) => event.action === "operator_portal.account_bootstrap_requested")
    );
    assert.ok(
      audit.some((event) => event.action === "operator_portal.account_bootstrap_evidence_recorded")
    );
    assert.ok(
      audit.some(
        (event) => event.action === "operator_portal.account_bootstrap_promoted_to_factual_test"
      )
    );
  } finally {
    await close();
...
test("Step 3.72 account bootstrap rejects phone numbers, OTP fields, seeds and incomplete PASS", async () => {
  ...
  try {
    const client = await loginClient(baseUrl);
    const seeded = await seedOperator(client);
    const rejectedSecret = await operatorRequest(
      baseUrl,
      seeded.session.token,
      "/operator-api/account-bootstrap/sessions",
      {
    ...
    const rejectedPass = await operatorRequest(
      baseUrl,
      seeded.session.token,
      `/operator-api/account-bootstrap/sessions/${created.payload.session.id}/evidence`,
      {
    ...
    const rejectedOtp = await operatorRequest(
      baseUrl,
      seeded.session.token,
      `/operator-api/account-bootstrap/sessions/${created.payload.session.id}/evidence`,
      {
    ...
    const rejectedSecretNote = await operatorRequest(
      baseUrl,
      seeded.session.token,
      `/operator-api/account-bootstrap/sessions/${created.payload.session.id}/evidence`,
      {
