...
test("Step 3.34 live Hetzner workload binds UI services to the private operator network", async () => {
  ...
  assert.match(startScript, /CHROME_CLI='--disable-session-crashed-bubble --no-first-run https:\/\/mail\.proton\.me\/'/);
  assert.match(startScript, /CHROME_CLI='--disable-session-crashed-bubble --no-first-run https:\/\/simplex\.chat\/downloads\/'/);
