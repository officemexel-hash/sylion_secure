...
async function clickSensitiveButton(page, label, toastText, actionLabel) {
  ...
  try {
    ...
    const toast = await page
      ...
      .catch(() => "");
    throw new Error(
      `Timed out waiting for toast "${toastText}" after ${label}. Current toast: "${toast}"`,
      { cause: error }
    );
  }
  return stepUpAction;
...
async function run() {
  ...
  try {
    ...
    const tenant = await latestByName(
      page,
      "/tenants",
      "tenants",
      (item) => item.name === `Step 3.28 Tenant ${runId}`
    );
    ...
    await waitForDashboardIdle(page);
    const operator = await latestByName(
      page,
      "/operators",
      "operators",
      (item) => item.displayName === `Step 3.28 Operator ${runId}`
    );
    if (!operator) throw new Error("Created operator not found after dashboard click");
    const pipelinesAfterOperator = await apiInPage(
      page,
      `/operator-provisioning/pipelines?operatorId=${encodeURIComponent(operator.id)}`
    );
    const localBaseline = [...pipelinesAfterOperator.pipelines]
      .reverse()
      .find((item) => item.status === "local_lab_ready");
    ...
    actions.push(
      await clickSensitiveButton(
        page,
        "Save Provider",
        "Provider saved; secret cleared from form",
        "provider_save"
      )
    );
    await waitForDashboardIdle(page);
    const provider = await latestByName(
      page,
      "/providers",
      "providers",
      (item) => item.providerKey === "hetzner"
    );
    ...
    await fill(
      page,
      "#approval-form input[name='evidenceRefs']",
      "release://step3-28/human-live-promotion"
    );
    await clickButton(page, "Create Approval");
    await waitForToast(page, "Provisioning approval created");
    await waitForDashboardIdle(page);
    const approval = await latestByName(
      page,
      "/provisioning/approvals",
      "approvals",
      (item) => item.reasonCode === `step3_28_live_promotion_${runId}`
    );
    ...
    await selectValue(
      page,
      "#approval-status-form select[name='status']",
      "approved_for_execution"
    );
    await fill(
      page,
      "#approval-status-form input[name='note']",
      "Step 3.28 human dashboard promotion approval"
    );
    ...
    if (approved?.status !== "approved_for_execution")
      throw new Error("Approval was not updated to approved_for_execution");
    ...
    await fill(
      page,
      "#baseline-promotion-form input[name='idempotencyKey']",
      `step3-28-promote-${runId}`
    );
    ...
    actions.push(
      await clickSensitiveButton(
        ...
        "Operator baseline promotion recorded with gate decision",
        "baseline_promotion"
      )
    );
    ...
    const request = [...requestsPayload.requests]
      .reverse()
      .find((item) => item.idempotencyKey === `step3-28-promote-${runId}`);
    ...
    const relatedAudit = auditPayload.events.filter(
      (item) =>
        item.idempotencyKey === request.idempotencyKey ||
        item.resourceId === request.id ||
        item.resourceId === request.rollbackPlanId
    );
    ...
    if (request.productionExecutionAllowed !== false)
      issues.push("promotion_request_production_execution_not_false");
    if (request.requestedResources?.join(",") !== "G1,G2,WORKLOAD")
      issues.push("promotion_request_wrong_resource_shape");
    if (rollbackPlan.actions?.length !== 3) issues.push("rollback_plan_does_not_cover_three_roles");
    if (
      !relatedAudit.some((item) =>
        ["live_cloud.vps_set_blocked", "live_cloud.vps_set_created"].includes(item.action)
      )
    ) {
    ...
    summary = {
      ...
      screenshots: screenshots.map((item) =>
        item.replace(process.cwd(), ".").replaceAll("\\", "/")
      ),
      actions,
      issues,
  ...
  if (issues.length) {
    await writeFile(
      ...
      JSON.stringify({ ...summary, status: "failed", issues }, null, 2)
    );
