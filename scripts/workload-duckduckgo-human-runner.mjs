...
import { evaluateDuckDuckGoFactualState } from "./lib/duckduckgo-factual-evaluator.mjs";
import { createAdminApiClient } from "./lib/admin-api-client-factory.mjs";

const appKey = "duckduckgo_browser";
...
function markerFromText(text) {
  ...
  if (/chromium|chrome|firefox/i.test(normalized)) return "generic_browser";
  return null;
}

async function loginClient() {
  const anon = createAdminApiClient(baseUrl, "corr_step3_86_duck_");
  const credentialId = `cred-step3-86-duck-${crypto.randomUUID()}`;
  try {
...
async function operatorRequest(token, path, { method = "GET", body } = {}) {
  const response = await fetch(`${baseUrl}${path}`, {
    ...
    headers: {
      ...
      "x-correlation-id": `corr_step3_86_duck_operator_${crypto.randomUUID()}`
    },
...
function manualVisualProbe() {
  const marker = process.env.SYLION_DUCKDUCKGO_UI_MARKER;
  const evidenceRef = process.env.SYLION_DUCKDUCKGO_UI_EVIDENCE_REF;
  if (!marker && !evidenceRef) return null;
  return {
    status: process.env.SYLION_DUCKDUCKGO_UI_STATUS || "passed",
    marker,
    evidenceRef,
...
function manualBrowsingProbe() {
  const status = process.env.SYLION_DUCKDUCKGO_BROWSING_STATUS;
  const evidenceRef = process.env.SYLION_DUCKDUCKGO_BROWSING_EVIDENCE_REF;
  if (!status && !evidenceRef) return null;
  return {
    status: status || "passed",
    throughWorkloadRoute: envBool("SYLION_DUCKDUCKGO_THROUGH_WORKLOAD_ROUTE"),
    targetHost: process.env.SYLION_DUCKDUCKGO_BROWSING_HOST || "example.org",
    evidenceRef,
    mode: "human_pixel_or_laptop_browsing_probe",
    evidenceArtifactIds: evidenceRef?.startsWith("artifact://") ? [evidenceRef] : [],
...
function routeProbeFromEnv() {
  if (!process.env.SYLION_DUCKDUCKGO_ROUTE_PROBE) return null;
  return {
    dnsThroughTunnel: envBool("SYLION_DUCKDUCKGO_DNS_THROUGH_TUNNEL"),
    terminalDefaultRoute: process.env.SYLION_DUCKDUCKGO_TERMINAL_DEFAULT_ROUTE || "unknown",
    workloadEgress: process.env.SYLION_DUCKDUCKGO_WORKLOAD_EGRESS || "unknown",
    evidenceArtifactIds: process.env.SYLION_DUCKDUCKGO_ROUTE_EVIDENCE_REF?.startsWith("artifact://")
      ? [process.env.SYLION_DUCKDUCKGO_ROUTE_EVIDENCE_REF]
      : []
  };
}
...
async function visualProbeFromBrowser({ operatorToken, streamSession }) {
  ...
  try {
    ...
    if (openStream && streamSession?.state === "stream_session_ready" && streamSession?.launchUrl) {
      ...
      probe.marker = marker;
      probe.status = marker === "duckduckgo_search" ? "passed" : marker ? "failed" : "blocked";
      probe.evidenceRef = `screenshot:${repoRelativePath(screenshots.stream)}`;
      probe.evidenceArtifactIds = [`artifact://local/step3-86-duckduckgo-stream-view`];
...
async function run() {
  ...
  const browsingProbe = manualBrowsingProbe();
  const routeProbe = routeProbeFromEnv();
  const evaluation = evaluateDuckDuckGoFactualState({
    matrixItem,
    terminalMode,
    runtimeMode,
    streamSession: streamPayload.session,
