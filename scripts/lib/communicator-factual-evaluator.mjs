const COMMUNICATOR_APPS = Object.freeze({
  ...
  whatsapp: {
    ...
    rejectedMarkers: ["web_whatsapp_login_only", "generic_browser", "download_page", "qr_only", "new_tab", "about_blank"],
    requiredChecks: ["uiVisible", "accountBootstrap", "sendReceive"]
  },
  telegram: {
    label: "Telegram",
    acceptedMarkers: ["telegram", "telegram_desktop", "telegram_android"],
    rejectedMarkers: ["telegram_download_page", "generic_browser", "download_page", "phone_prompt_only", "new_tab", "about_blank"],
    requiredChecks: ["uiVisible", "accountBootstrap", "sendReceive"]
  },
  threema: {
    label: "Threema",
    acceptedMarkers: ["threema", "threema_desktop", "threema_android"],
    rejectedMarkers: ["threema_download_page", "generic_browser", "download_page", "new_tab", "about_blank"],
    requiredChecks: ["uiVisible", "accountBootstrap", "sendReceive"]
  },
  zangi: {
    label: "Zangi",
...
const ALLOWED_RUNTIME_MODES = new Set(["desktop", "android_native", "firecracker_gui", "container"]);
const ALLOWED_GUARDRAIL_KEYS = new Set(["nonSecretBootstrap", "metadataOnly", "communicationDataStored"]);
...
function forbiddenProbeFields(input, path = []) {
  ...
  return Object.entries(input).flatMap(([key, nested]) => {
    ...
    const own = ALLOWED_GUARDRAIL_KEYS.has(key) ? [] : /(password|secret|token|api_key|otp|sms|verification_code|phone_number|seed|mnemonic|private_key|message_content|chat_content|contact_list)/.test(normalized)
      ? [current.join(".")]
      : [];
...
export function evaluateCommunicatorFactualState({
  ...
  const forbiddenFields = forbiddenProbeFields({ visualProbe, accountProbe, sendReceiveProbe, apkProbe });
  ...
  const accountPassed = accountProbe?.status === "passed"
  ...
  const sendReceivePassed = sendReceiveProbe?.status === "passed"
  ...
  const checks = {
    uiVisible: visualPassed
      ? check("passed", `${definition.label} UI marker observed through workload stream`, visualProbe?.note || null, visualProbe?.mode || null)
      : check(failed ? "failed" : "blocked", null, markerRejected(definition, visualProbe?.marker)
    ...
    accountBootstrap: accountPassed
      ? check("passed", `${definition.label} non-secret account bootstrap metadata passed`, accountProbe?.note || null, accountProbe?.mode || null)
      : check(failed ? "failed" : "blocked", null, "Account bootstrap is not proven without secrets or web-link-only evidence"),
    sendReceive: sendReceivePassed && routePassed
      ? check("passed", `${definition.label} metadata-only send/receive check passed through workload route`, sendReceiveProbe?.note || null, sendReceiveProbe?.mode || null)
