...
test("V2 admin web shell is served from Admin API under /admin", async () => {
  ...
  try {
    ...
    assert.match(html.body, /id="operator-commercial-summary-cards"/);
    assert.match(html.body, /id="operator-segmentation-table"/);
    assert.match(html.body, /id="metric-operator-infra-cost"/);
